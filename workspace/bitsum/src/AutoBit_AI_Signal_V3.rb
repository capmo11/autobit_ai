...
class AutoBit_AI_SignalV3
  ...
  def initialize(loginId,loginPw)
    ...
    @signalScore = 0 
    @finalSignalScore = 0
    @telegramChatId  = -1001189596297  # real Id  : -1001189596297     ,    test Id  ::  -1001364815243
    @profit = 0
    @positionValue = 0
  ...
  def getChartTimeStamp
       ...
       if ( timeSecond == "40" )
           ...
           if( timeSecond == "58"  && timeMinute == "59"   )
             return "1hour"
           else
             return ""   
           end 
...
'+text.to_s)
 ...
 def telegramSMS(text)
    begin
        uri = URI.parse( URI.encode('https://api.telegram.org/bot474059759:AAFM24fbUFVyEhFN6bCszocGgdUqWyJqbxA/sendmessage?chat_id='+@telegramChatId.to_s+'&text=비트코인  가격기준
'+text+'') )
        Net::HTTP.get(uri)
    rescue Exception => e
    ...
    begin
     ...
     # initKrChartDataSetting  ##TODO 만들어야함
                ...
                while true
                          ...
                          if(init == 0 || getChartTimeStamp =="1hour" )
                                       ...
                                       @krChartDTO_5Index =  krChartDataGetting(@interVal_5)
                                       ...
                                       telegramSMS("****** Data reflesh Start....*******")
                                      ...
                                      result3 =  buyOrSell_Signal_CalCulate(@krChartDTO_before_3Index ,@krChartDTO_3Index, @interVal_3, 0 )
                                      result4 =  buyOrSell_Signal_CalCulate(@krChartDTO_before_4Index ,@krChartDTO_4Index, @interVal_4, 0 )
                                      result5 =  buyOrSell_Signal_CalCulate(@krChartDTO_before_5Index ,@krChartDTO_5Index, @interVal_5, 0 )
                                      score = 0
                            ...
                            if (score > 10)
                                 text ='AutoBit Signal *****
30분 시그널 : '+scoreResultText(result1)+'                                 
1시간 시그널 : '+scoreResultText(result2)+'                                 
4시간 시그널 : '+scoreResultText(result3)+'                                 
12시간 시그널 : '+scoreResultText(result4)+'                                 
1일  시그널 : '+scoreResultText(result5)+'                                 
종합결과 : '+totalScoreResultText(score)+'
***********************'
                                  telegramSMS(text)
                            else
                              text ='AutoBit Signal *****
...
doTask = AutoBit_AI_SignalV3.new('capmo1@naver.com','Dnjzm1324!') 
doTask.loginDo
